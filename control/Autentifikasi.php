...

{
    ...
    { $this->load->library('session');
      if($this->session->has_userdata('email')){redirect('user');}
        ...
        $user = $this->ModelUser->cekData(['email' => $email])->row_array();
                if ($user) {  
                    if ($user['is_active'] == 1) { 
                     ...
                     { 
                         redirect('admin'); 
                    } else {  if ($user['image'] == 'default.jpg') {  $this->session->set_flashdata('pesan', '<div class="alert alert-info alert-message" role="alert">Silahkan 
